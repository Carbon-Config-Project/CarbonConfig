...
public class ColorElement extends ValueElement
{
	...
	public void renderRightPart(int left, int top, int desiredWidth, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
		...
		BaseCarbonScreen.fill(left, top, left+20, top+height-(hasAlpha ? 1 : 0), color);
		...
		BaseCarbonScreen.fill(left+23, top+1, red.x-2, top+red.getHeight()-1, (int)(0xFF000000 | (red.get() & 0xFF) << 16));
		...
		BaseCarbonScreen.fill(left+23, top+1, green.x-2, top+green.getHeight()-1, (int)(0xFF000000 | (green.get() & 0xFF) << 8));
		...
		BaseCarbonScreen.fill(left+23, top+1, blue.x-2, top+blue.getHeight()-1, (int)(0xFF000000 | (blue.get() & 0xFF)));
		...
		if(hasAlpha) {
			...
			BaseCarbonScreen.fill(left+23, top+1, alpha.x-2, top+alpha.getHeight()-1, (int)(0xFF000000 | r << 16 | g << 8 | b));
