...
public class ColorElement extends ValueElement
{
	...
	public void renderRightPart(int left, int top, int desiredWidth, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
		...
		if(isForcingText()) {
			...
			text.xPosition = left + xOff;
			text.yPosition = top + 1;
		...
		red.xPosition = left + xOff;
		red.yPosition = top;
		red.setWidth(realWidth);
		red.setHeight(realHeight);
		red.render(mouseX, mouseY, partialTicks);
		BaseCarbonScreen.drawRect(left+23, top+1, red.xPosition-2, top+red.getHeight()-1, (int)(0xFF000000 | (red.get() & 0xFF) << 16));
		GuiUtils.drawFrame(left+22, top, red.xPosition-2, top+red.getHeight()-1, 0xFF848484, 1F);
		top += red.getHeight()+1;
		
		green.xPosition = left + xOff;
		green.yPosition = top;
		green.setWidth(realWidth);
		green.setHeight(realHeight);
		green.render(mouseX, mouseY, partialTicks);
		BaseCarbonScreen.drawRect(left+23, top+1, green.xPosition-2, top+green.getHeight()-1, (int)(0xFF000000 | (green.get() & 0xFF) << 8));
		GuiUtils.drawFrame(left+22, top, green.xPosition-2, top+green.getHeight()-1, 0xFF848484, 1F);
		top += green.getHeight()+1;
		
		blue.xPosition = left + xOff;
		blue.yPosition = top;
		blue.setWidth(realWidth);
		blue.setHeight(realHeight);
		blue.render(mouseX, mouseY, partialTicks);
		BaseCarbonScreen.drawRect(left+23, top+1, blue.xPosition-2, top+blue.getHeight()-1, (int)(0xFF000000 | (blue.get() & 0xFF)));
		GuiUtils.drawFrame(left+22, top, blue.xPosition-2, top+blue.getHeight()-1, 0xFF848484, 1F);
		top += blue.getHeight()+1;
		
		if(hasAlpha) {
			alpha.xPosition = left + xOff;
			alpha.yPosition = top;
			...
			BaseCarbonScreen.drawRect(left+23, top+1, alpha.xPosition-2, top+alpha.getHeight()-1, (int)(0xFF000000 | r << 16 | g << 8 | b));
			GuiUtils.drawFrame(left+22, top, alpha.xPosition-2, top+alpha.getHeight()-1, 0xFF848484, 1F);
			top += alpha.getHeight()+1;
		}
		
		text.xPosition = left + 23;
		text.yPosition = top+1;
