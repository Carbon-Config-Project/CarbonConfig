...
public class ColorElement extends ValueElement
{
	...
	public void renderRightPart(MatrixStack stack, int left, int top, int desiredWidth, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
		...
		BaseCarbonScreen.fill(stack, left+23, top+1, red.x-2, top+red.getHeight()-1, (int)(0xFF000000 | (red.get() & 0xFF) << 16));
		GuiUtils.drawFrame(stack, left+22, top, red.x-2, top+red.getHeight()-1, 0xFF848484, 1F);
		top += red.getHeight()+1;
		...
		BaseCarbonScreen.fill(stack, left+23, top+1, green.x-2, top+green.getHeight()-1, (int)(0xFF000000 | (green.get() & 0xFF) << 8));
		GuiUtils.drawFrame(stack, left+22, top, green.x-2, top+green.getHeight()-1, 0xFF848484, 1F);
		...
		BaseCarbonScreen.fill(stack, left+23, top+1, blue.x-2, top+blue.getHeight()-1, (int)(0xFF000000 | (blue.get() & 0xFF)));
		GuiUtils.drawFrame(stack, left+22, top, blue.x-2, top+blue.getHeight()-1, 0xFF848484, 1F);
		...
		if(hasAlpha) {
			...
			BaseCarbonScreen.fill(stack, left+23, top+1, alpha.x-2, top+alpha.getHeight()-1, (int)(0xFF000000 | r << 16 | g << 8 | b));
			GuiUtils.drawFrame(stack, left+22, top, alpha.x-2, top+alpha.getHeight()-1, 0xFF848484, 1F);
