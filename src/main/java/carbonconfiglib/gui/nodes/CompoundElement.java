...
public class CompoundElement extends NodeElement implements IFolderNode
{
	...
	public ITextComponent getName() {
		if(shouldRenderIndex()) {
			ITextComponent comp = getOverride();
	...
	public void renderLeftPart(MatrixStack stack, int left, int top, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
		ITextComponent comp = shouldRenderIndex() ? getOverride() : null;
	...
	private <T> ITextComponent getOverride() {
	...
	public void renderRightPart(MatrixStack stack, int left, int top, int desiredWidth, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
		GuiUtils.drawScrollingShadowText(stack, font, Texts.translatable("gui.carbonconfig.elements", node.getValues().size()), left+(desiredWidth>>1), top, (desiredWidth>>1)-2, height, Align.END, -1, sinceFullyVisible);
	}
