...
public class FolderElement extends BaseElement implements IFolderNode
{
	...
	public void renderRightPart(MatrixStack stack, int left, int top, int desiredWidth, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
		GuiUtils.drawScrollingShadowText(stack, font, Texts.translatable("gui.carbonconfig.elements", node.getChildren().size()), left, top, desiredWidth-2, height, Align.END, -1, sinceFullyVisible);
	}
