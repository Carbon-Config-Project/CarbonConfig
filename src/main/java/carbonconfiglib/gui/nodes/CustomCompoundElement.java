...
public class CustomCompoundElement extends NodeElement
{
	...
	public void renderLeftPart(MatrixStack stack, int left, int top, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
		GuiUtils.drawScrollingShadowText(stack, font, shouldRenderIndex() ? Texts.literal(index(node)+": ") : node.getName(), left, top, width, height, Align.START, -1, sinceFullyVisible);
	}
