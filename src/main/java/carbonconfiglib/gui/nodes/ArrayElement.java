...
public class ArrayElement extends NodeElement implements IFolderNode, ISortableNode
{
	...
	@Override
	public void renderLeftPart(MatrixStack stack, int left, int top, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
		...
		button.setMessage(Texts.literal(active ? "◀" : "▶"));
	...
	public void renderRightPart(MatrixStack stack, int left, int top, int desiredWidth, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
		GuiUtils.drawScrollingShadowText(stack, font, Texts.translatable("gui.carbonconfig.elements", node.size()), left, top, desiredWidth-2, height, Align.END, -1, sinceFullyVisible);
	}
	...
	public static class AddElement extends BaseElement {
		...
		public AddElement(ArrayElement owner) {
			...
			selector.setMessage(Texts.translatable("gui.carbonconfig.array.new"));
		...
		@Override
		public void renderLeftPart(MatrixStack stack, int left, int top, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
			GuiUtils.drawScrollingShadowText(stack, font, Texts.translatable("gui.carbonconfig.array.next"), left, top, width, height, Align.START, -1, 0);
		}
