...
{
	protected DropDownState<Suggestion> state = new DropDownState<Suggestion>(T -> Texts.literal(T.getName()), ObjectLists.empty()).valueOnly(true).allowEmpty(false).withListener(this::onSelectionChanged);
	...
	protected void setRightComponentsVisible(boolean value) {
		...
		text.setVisible(!values.visible && value);
	}
	...
	public void renderRightPart(int left, int top, int desiredWidth, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
		values.xPosition = left;
		values.yPosition = top;
		values.setWidth(desiredWidth);
		values.setHeight(height);
		values.render(mouseX, mouseY, partialTicks);
