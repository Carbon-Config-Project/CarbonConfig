...
public abstract class NumberElement extends ValueElement
{
	...
	private void updateState() {
		...
		text.setVisible(!slider.visible);
	}
	...
	protected void setRightComponentsVisible(boolean value) {
		...
		text.setVisible(value && !slider.visible);
	...
	public void renderRightPart(int left, int top, int desiredWidth, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
		slider.xPosition = left;
		slider.yPosition = top;
		...
		if(slider.getState().getRange() > 0 && width - (desiredWidth+2) >= height) {
			subMode.xPosition = left + desiredWidth+2;
			subMode.yPosition = Align.CENTER.alignStart(top, height, subMode.getHeight());
			subMode.setWidth(height);
			subMode.setHeight(height);
			subMode.render(mouseX, mouseY, partialTicks);
		}
	}
