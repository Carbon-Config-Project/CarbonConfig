...
public class BackupSelectionScreen extends BaseCarbonScreen
{
	...
	public BackupSelectionScreen(Screen parent, BackgroundHolder holder, IModConfig config, List<BackupEntry> entries) {
		this.parent = parent;
		this.holder = holder;
		header = Texts.literal(ModList.get().getModContainerById(config.getModId()).map(ModContainer::getModInfo).map(IModInfo::getDisplayName).orElse(config.getModId())).appendText(" -> ").appendText(config.getConfigName());
	...
	public void renderForeground(int mouseX, int mouseY, float partialTicks) {
		...
		GuiUtils.drawScrollingShadowText(font, Texts.translatable("gui.carbonconfig.backup.header"), 0, 0, width, (int)(height * 0.15F)+20, Align.CENTER, -1, 0);
	}
	...
	public static class BackupListEntry extends ListEntry<BackupListEntry> {
		...
		private void load() {
			...
			if(CarbonConfig.BACKUP_TOASTS.get()) Minecraft.getInstance().getToastGui().add(new SystemToast(SystemToast.Type.TUTORIAL_HINT, Texts.translatable("gui.carbonconfig.toast.load"), Texts.translatable("gui.carbonconfig.toast.load.desc")));
			owner.onClose();
		}
		...
		public void render(int x, int top, int left, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
			...
			GuiUtils.drawText(font, Texts.translatable("gui.carbonconfig.backup.created", createDuration(Duration.between(entry.created(), LocalDateTime.now()))).applyTextStyle(TextFormatting.GRAY), left+25, top+font.FONT_HEIGHT+2, Align.START, -1);
			GuiUtils.drawText(font, Texts.translatable("gui.carbonconfig.backup.size", findBestMemory(entry.fileSize(), 1000), findBestMemory(entry.compressedSize(), 1000), ((int)(entry.getRatio()*100))+"%").applyTextStyle(TextFormatting.DARK_AQUA), left+25, top+font.FONT_HEIGHT*2+2, Align.START, -1);
