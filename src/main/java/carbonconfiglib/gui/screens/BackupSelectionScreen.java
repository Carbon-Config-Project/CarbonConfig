...
public class BackupSelectionScreen extends BaseCarbonScreen
{
	...
	public BackupSelectionScreen(Screen parent, BackgroundHolder holder, IModConfig config, List<BackupEntry> entries) {
		this.parent = parent;
		this.holder = holder;
		header = Texts.literal(FabricLoader.getInstance().getModContainer(config.getModId()).map(ModContainer::getMetadata).map(ModMetadata::getName).orElse(config.getModId())).append(" -> ").append(config.getConfigName());
