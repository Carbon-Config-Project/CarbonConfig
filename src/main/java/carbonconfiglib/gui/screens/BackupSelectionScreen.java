...
public class BackupSelectionScreen extends BaseCarbonScreen
{
	...
	public void renderForeground(int mouseX, int mouseY, float partialTicks) {
		...
		GuiUtils.drawScrollingShadowText(fontRendererObj, Texts.translatable("gui.carbonconfig.backup.header"), 0, 0, width, (int)(height * 0.15F)+20, Align.CENTER, -1, 0);
	}
...
//			if(CarbonConfig.BACKUP_TOASTS.get()) Minecraft.getMinecraft().getToastGui().add(new SystemToast(SystemToast.Type.TUTORIAL_HINT, Texts.translatable("gui.carbonconfig.toast.load"), Texts.translatable("gui.carbonconfig.toast.load.desc")));
		...
		public void render(int x, int top, int left, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
			...
			GuiUtils.drawText(font, Texts.translatable("gui.carbonconfig.backup.created", createDuration(Duration.between(entry.created(), LocalDateTime.now()))).setChatStyle(Texts.applyStyle(EnumChatFormatting.GRAY)), left+25, top+font.FONT_HEIGHT+2, Align.START, -1);
			GuiUtils.drawText(font, Texts.translatable("gui.carbonconfig.backup.size", findBestMemory(entry.fileSize(), 1000), findBestMemory(entry.compressedSize(), 1000), ((int)(entry.getRatio()*100))+"%").setChatStyle(Texts.applyStyle(EnumChatFormatting.DARK_AQUA)), left+25, top+font.FONT_HEIGHT*2+2, Align.START, -1);
			
			int right = left + width;
			load.xPosition = right - 39;
			load.yPosition = (int)Align.CENTER.alignStart(top, height, 18);
			load.render(mouseX, mouseY, partialTicks);
			delete.xPosition = right - 20;
			delete.yPosition = (int)Align.CENTER.alignStart(top, height, 18);
			delete.render(mouseX, mouseY, partialTicks);
		}
		
		private IChatComponent createDuration(Duration duration) {
