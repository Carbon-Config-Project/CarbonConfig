...
public class BackupSelectionScreen extends BaseCarbonScreen
{
	...
	public BackupSelectionScreen(Screen parent, BackgroundHolder holder, IModConfig config, List<BackupEntry> entries) {
		this.parent = parent;
		this.holder = holder;
		header = Component.literal(FabricLoader.getInstance().getModContainer(config.getModId()).map(ModContainer::getMetadata).map(ModMetadata::getName).orElse(config.getModId())).append(" -> ").append(config.getConfigName());
	...
	public static class BackupListEntry extends ListEntry<BackupListEntry> {
		...
		public void render(PoseStack poseStack, int x, int top, int left, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
			...
			font.draw(poseStack, Component.translatable("gui.carbonconfig.backup.entry", entry.getFileName()), left+25, top+2, -1);
			
			font.draw(poseStack, Component.translatable("gui.carbonconfig.backup.created", createDuration(Duration.between(entry.created(), LocalDateTime.now()))).withStyle(ChatFormatting.GRAY), left+25, top+font.lineHeight+2, -1);
			font.draw(poseStack, Component.translatable("gui.carbonconfig.backup.size", findBestMemory(entry.fileSize(), 1000), findBestMemory(entry.compressedSize(), 1000), ((int)(entry.getRatio()*100))+"%").withStyle(ChatFormatting.DARK_AQUA), left+25, top+font.lineHeight*2+2, -1);
