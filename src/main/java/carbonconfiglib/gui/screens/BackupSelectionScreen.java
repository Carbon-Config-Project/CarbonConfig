...
public class BackupSelectionScreen extends BaseCarbonScreen
{
	...
	public static class BackupListEntry extends ListEntry<BackupListEntry> {
		...
		private void load() {
			...
			if(CarbonConfig.BACKUP_TOASTS.get()) Minecraft.getInstance().getToasts().addToast(new SystemToast(SystemToast.Type.TUTORIAL_HINT, Texts.translatable("gui.carbonconfig.toast.load"), Texts.translatable("gui.carbonconfig.toast.load.desc")));
			owner.onClose();
		}
		...
		public void render(MatrixStack poseStack, int x, int top, int left, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
			...
			font.draw(poseStack, Texts.translatable("gui.carbonconfig.backup.created", createDuration(Duration.between(entry.created(), LocalDateTime.now()))).withStyle(TextFormatting.GRAY), left+25, top+font.lineHeight+2, -1);
			font.draw(poseStack, Texts.translatable("gui.carbonconfig.backup.size", findBestMemory(entry.fileSize(), 1000), findBestMemory(entry.compressedSize(), 1000), ((int)(entry.getRatio()*100))+"%").withStyle(TextFormatting.DARK_AQUA), left+25, top+font.lineHeight*2+2, -1);
