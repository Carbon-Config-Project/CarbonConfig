	...
	public static class WorldElement extends ListEntry<WorldElement> {
		...
		public WorldElement(IModConfig config, IConfigTarget target, BackgroundHolder holder, GuiScreen parent) {
			...
			if(target instanceof WorldConfigTarget) {
				...
				path = Texts.literal(sum.getFileName()).setStyle(Texts.applyStyle(TextFormatting.GRAY));
			}
			else {
				title = Texts.literal(target.getName());
				Path folder = target.getFolder();
				int index = folder.getNameCount();
				path = Texts.literal(folder.subpath(index-3, index).toString()).setStyle(Texts.applyStyle(TextFormatting.GRAY));
			}
		...
		public void init() {
			...
			if(target instanceof WorldConfigTarget) {
				loadIcon(Minecraft.getMinecraft().getSaveLoader().getFile(((WorldConfigTarget)target).getSummary().getFileName(), "icon.png").toPath());
			}
		}
