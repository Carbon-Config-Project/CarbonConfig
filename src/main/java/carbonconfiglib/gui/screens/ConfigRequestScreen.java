...
{
	static final IChatComponent REQUEST = Texts.translatable("gui.carbonconfig.requesting_config");
	static final IChatComponent[] ANIMATION = new IChatComponent[] {
			Texts.literal("Ooooo").setChatStyle(Texts.applyStyle(EnumChatFormatting.GRAY)),
			Texts.literal("oOooo").setChatStyle(Texts.applyStyle(EnumChatFormatting.GRAY)),
			Texts.literal("ooOoo").setChatStyle(Texts.applyStyle(EnumChatFormatting.GRAY)),
			Texts.literal("oooOo").setChatStyle(Texts.applyStyle(EnumChatFormatting.GRAY)),
			Texts.literal("ooooO").setChatStyle(Texts.applyStyle(EnumChatFormatting.GRAY)),
	...
	public ConfigRequestScreen(BackgroundHolder customTexture, GuiScreen parent, IModConfig config, String...walker) {
	...
	public void receiveConfigData(UUID requestId, PacketBuffer buf) {
		...
		if(result.test(buf)) {
			mc.displayGuiScreen(new ConfigScreen(config, texture, parent).withWalker(walker == null || walker.length <= 0 ? null : ObjectArrayList.wrap(walker)));
			return;
		}
	...
	public void renderBackground(int mouseX, int mouseY, float partialTicks) {
		...
		if(!texture.shouldDisableInLevel() || mc.theWorld == null) GuiUtils.renderBackground(0, width, 0, height, 0F, texture.getTexture());
	...
	public void renderForeground(int mouseX, int mouseY, float partialTicks) {
		...
		drawText(REQUEST, 0, -15, Align.CENTER, -1);
		int index = (tick / 5) % 8;
		if(index >= 5) index = 8-index;
		drawText(ANIMATION[index], 0, -3, Align.CENTER, -1);
		int timeout = (401 - tick) / 20;
		if(timeout > 18) return;
		drawText( Texts.translatable("gui.carbonconfig.timeout", timeout).setChatStyle(Texts.applyStyle(EnumChatFormatting.RED)), 0, 9, Align.CENTER, -1);
