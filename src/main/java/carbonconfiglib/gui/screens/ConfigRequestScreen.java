...
{
	static final Component REQUEST = Texts.translatable("gui.carbonconfig.requesting_config");
	static final Component[] ANIMATION = new Component[] {
			Texts.literal("Ooooo").withStyle(ChatFormatting.GRAY),
			Texts.literal("oOooo").withStyle(ChatFormatting.GRAY),
			Texts.literal("ooOoo").withStyle(ChatFormatting.GRAY),
			Texts.literal("oooOo").withStyle(ChatFormatting.GRAY),
			Texts.literal("ooooO").withStyle(ChatFormatting.GRAY),
	...
	public void renderForeground(PoseStack matrix, int mouseX, int mouseY, float partialTicks) {
		...
		drawText(matrix,  Texts.translatable("gui.carbonconfig.timeout", timeout).withStyle(ChatFormatting.RED), 0, 9, Align.CENTER, -1);
