...
public class ConfigRequestScreen extends BaseCarbonScreen implements IRequestReceiver
{
	...
	static final ITextComponent[] ANIMATION = new ITextComponent[] {
			Texts.literal("Ooooo").withStyle(TextFormatting.GRAY),
			Texts.literal("oOooo").withStyle(TextFormatting.GRAY),
			Texts.literal("ooOoo").withStyle(TextFormatting.GRAY),
			Texts.literal("oooOo").withStyle(TextFormatting.GRAY),
			Texts.literal("ooooO").withStyle(TextFormatting.GRAY),
	...
	public void renderForeground(MatrixStack matrix, int mouseX, int mouseY, float partialTicks) {
		...
		drawText(matrix, REQUEST, 0, -15, Align.CENTER, -1);
		int index = (tick / 5) % 8;
		if(index >= 5) index = 8-index;
		drawText(matrix, ANIMATION[index], 0, -3, Align.CENTER, -1);
		int timeout = (401 - tick) / 20;
		if(timeout > 18) return;
		drawText(matrix,  Texts.translatable("gui.carbonconfig.timeout", timeout).withStyle(TextFormatting.RED), 0, 9, Align.CENTER, -1);
