...
public class ConfigRequestScreen extends BaseCarbonScreen implements IRequestReceiver
{
	...
	static final ITextComponent[] ANIMATION = new ITextComponent[] {
			Texts.literal("Ooooo").applyTextStyle(TextFormatting.GRAY),
			Texts.literal("oOooo").applyTextStyle(TextFormatting.GRAY),
			Texts.literal("ooOoo").applyTextStyle(TextFormatting.GRAY),
			Texts.literal("oooOo").applyTextStyle(TextFormatting.GRAY),
			Texts.literal("ooooO").applyTextStyle(TextFormatting.GRAY),
	...
	public void receiveConfigData(UUID requestId, PacketBuffer buf) {
		...
		if(result.test(buf)) {
			minecraft.displayGuiScreen(new ConfigScreen(config, texture, parent).withWalker(walker == null || walker.length <= 0 ? null : ObjectArrayList.wrap(walker)));
			return;
		}
	...
	public void renderBackground(int mouseX, int mouseY, float partialTicks) {
		...
		if(!texture.shouldDisableInLevel() || minecraft.world == null) GuiUtils.renderBackground(0, width, 0, height, 0F, texture.getTexture());
	...
	public void renderForeground(int mouseX, int mouseY, float partialTicks) {
		...
		drawText( Texts.translatable("gui.carbonconfig.timeout", timeout).applyTextStyle(TextFormatting.RED), 0, 9, Align.CENTER, -1);
