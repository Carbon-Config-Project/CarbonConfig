...
public class ConfigScreen extends BaseCarbonScreen implements IElementContext, IFolderController
{
	...
	public ConfigScreen(IModConfig configs, BackgroundHolder holder, Screen parent) {
		...
		display = Component.literal(FabricLoader.getInstance().getModContainer(configs.getModId()).map(ModContainer::getMetadata).map(ModMetadata::getName).orElse(configs.getModId())).append(" -> ").append(configs.getConfigName());
	...
	public void drawForeground(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		if(rowTwo.isVisible() && !rowTwo.isScrollbarVisible() && !rowThree.isEmpty()) {
			graphics.fill(widthTwo, (int)(height * 0.15F), widthTwo+4, (int)(height * 0.8F), 0xFF000000);
		}
		if(rowThree.isVisible()) {
			int widthThree = (int)(width * totalWidth()) - 4;
			graphics.fill(widthThree, (int)(height * 0.15F), widthThree+4, (int)(height * 0.8F), 0xFF000000);
		}
		graphics.fill(0, (int)(height * 0.8F), width, height, 0xFF1C1C1C);
		boolean found = false;
		for(int i = 2;i>=0;i--) {
			if(all[i].isVisible()) {
				if(all[i].getOwner() != null && all[i].getOwner().getHovered() != null) {
					drawTooltip(all[i].getOwner().getHovered(), graphics);
					found = true;
					break;
				}
			}
		...
		if(!found && tooltipFocused != null) {
			drawTooltip(tooltipFocused, graphics);
		}
	...
	private void drawTooltip(BaseElement element, GuiGraphics graphics) {
		...
		if(text != null) {
			...
			stack.scale(scale, scale, 1F);
			GuiUtils.drawSplitText(graphics, font, text, 0F, 0F, Align.START, -1, (width / scale) - 4);
			stack.popPose();
		}
	}
