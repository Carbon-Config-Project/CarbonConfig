...
public class ConfigScreen extends BaseCarbonScreen implements IElementContext, IFolderController
{
	...
	CheckBoxState bulkEdit = new CheckBoxState(false, Icon.NOT_DEFAULT).setCallback(T -> onBulkEdit(T.getValue())).setTooltip(Texts.translatable("gui.carbonconfig.bulkedit"));
	CheckBoxState autoSave = new CheckBoxState(CarbonConfig.AUTO_SAVE.get(), Icon.AUTO_SAVE).setCallback(T -> onNodeChanged()).setTooltip(Texts.translatable("gui.carbonconfig.autosave"));
	CheckBoxState layerMode = new CheckBoxState(true, Icon.PAGE_MODE).setCallback(T -> recalculateNode()).withTooltip(T -> Texts.translatable(T.getState().getValue() ? "gui.carbonconfig.layout.normal" : "gui.carbonconfig.layout.wide"));
	CarbonButton save;
	...
	public ConfigScreen(IModConfig configs, BackgroundHolder holder, GuiScreen parent) {
		...
		display = Texts.literal(Optional.ofNullable(Loader.instance().getIndexedModList().get(configs.getModId())).map(ModContainer::getName).orElse(configs.getModId())).appendText(" -> ").appendText(configs.getConfigName());
	...
	public void initGui() {
		...
		iconButton(minY, minY - 22, 20, 20, Align.START, Align.START, Icon.HOME, T -> onClose()).withTooltip(Texts.translatable("gui.carbonconfig.home"));
		...
		save = iconButton(-49, minY - 20, 18, 18, Align.END, Align.START, Icon.SAVE, T -> save()).setPadding(0).withTooltip(Texts.translatable("gui.carbonconfig.save"));
	...
	public void onClose() {
		if(rootElement.needsSaving()) {
			...
			Texts.translatable("gui.carbonconfig.warn.changed"), 
			Texts.translatable("gui.carbonconfig.warn.changed.desc"),
			Texts.translatable("gui.carbonconfig.warn.changed.save"),
			Texts.translatable("gui.carbonconfig.warn.changed.discard"),
			Texts.translatable("gui.carbonconfig.warn.changed.back")));
			return;
		}
		setScreen(parent);
	}
	...
	public void renderBackground(int mouseX, int mouseY, float partialTicks) {
		if(save != null) save.enabled = rootElement.needsSaving();
		int minY = (int)(height * 0.15F);
	...
	public void renderForeground(int mouseX, int mouseY, float partialTicks) {
		drawText(display, 0, -centerY + 3, Align.CENTER, -1);
		...
		if(!rowOne.isScrollbarVisible() && !rowTwo.isEmpty()) {
			GuiScreen.drawRect(widthOne, (int)(height * 0.15F), widthOne+4, (int)(height * 0.8F), 0xFF000000);
		}
		int widthTwo = calculateWidth(1) + widthOne + 4;
		if(rowTwo.isVisible() && !rowTwo.isScrollbarVisible() && !rowThree.isEmpty()) {
			GuiScreen.drawRect(widthTwo, (int)(height * 0.15F), widthTwo+4, (int)(height * 0.8F), 0xFF000000);
		}
		if(rowThree.isVisible()) {
			int widthThree = (int)(width * totalWidth()) - 4;
			GuiScreen.drawRect(widthThree, (int)(height * 0.15F), widthThree+4, (int)(height * 0.8F), 0xFF000000);
		}
		GuiScreen.drawRect(0, (int)(height * 0.8F), width, height, 0xFF1C1C1C);
		boolean found = false;
		for(int i = 2;i>=0;i--) {
			if(all[i].isVisible()) {
				if(all[i].getOwner() != null && all[i].getOwner().getHovered() != null) {
					drawTooltip(all[i].getOwner().getHovered());
					found = true;
					break;
				}
			}
		...
		if(!found && tooltipFocused != null) {
			drawTooltip(tooltipFocused);
		}
		GuiScreen.drawRect(0, (int)(height * 0.85F), width, (int)(height * 0.85F+2), 0xFF000000);
	}
	...
	private void drawTooltip(BaseElement element) {
		...
		if(text != null) {
			...
			int scale = (int)((height * 0.85F) - (height * 0.8F)) / fontRenderer.FONT_HEIGHT;
			float minY = (height * 0.8F);
			float diff = (height * 0.85F - minY) * 0.5F;
			big = scale > 1;
			GlStateManager.pushMatrix();;
			GlStateManager.translate(2F, minY + diff - (fontRenderer.FONT_HEIGHT * scale * 0.5F), 0F);
			GlStateManager.scale(scale, scale, 1F);
			GuiUtils.drawText(fontRenderer, text, 0F, 0F, Align.START, -1);
			GlStateManager.popMatrix();
		}
		text = element.getTooltip();
		if(text != null) {
			int lines = fontRenderer.listFormattedStringToWidth(text.getFormattedText(), width>>1).size();
			int scale = ((int)(height - (height * 0.85F)) / (fontRenderer.FONT_HEIGHT << 1)) >= lines && big ? 2 : 1;
			float minY = (height * 0.85F) + 2F;
			GlStateManager.pushMatrix();;
			GlStateManager.translate(2F, minY, 0F);
			GlStateManager.scale(scale, scale, 1F);
			GuiUtils.drawSplitText(fontRenderer, text, 0F, 0F, Align.START, -1, (width / scale) - 4);
			GlStateManager.popMatrix();
		}
	}
	...
	private void notifyChanges(ReloadMode mode) {
		...
		GuiScreen owner = this;
		if(mode == ReloadMode.GAME) {
			mc.displayGuiScreen(new MultiChoiceScreen(T -> mc.displayGuiScreen(owner), Texts.translatable("gui.carbonconfig.restart.title"), Texts.translatable("gui.carbonconfig.restart.message").setStyle(Texts.applyStyle(TextFormatting.GRAY)), Texts.translatable("gui.carbonconfig.ok")));
		}
		else if(mode == ReloadMode.WORLD && mc.world != null) {
			mc.displayGuiScreen(new MultiChoiceScreen(T -> mc.displayGuiScreen(owner), Texts.translatable("gui.carbonconfig.reload.title"), Texts.translatable("gui.carbonconfig.reload.message").setStyle(Texts.applyStyle(TextFormatting.GRAY)), Texts.translatable("gui.carbonconfig.ok")));
		}
	}
}
