...
public class ConfigScreen extends BaseCarbonScreen implements IElementContext, IFolderController
{
	...
	private void drawTooltip(BaseElement element) {
		...
		if(text != null) {
			...
			GlStateManager.translate(2F, minY + diff - (fontRendererObj.FONT_HEIGHT * scale * 0.5F), 0F);
			GlStateManager.scale(scale, scale, 1F);
			GuiUtils.drawText(fontRendererObj, text, 0F, 0F, Align.START, -1);
	...
	private void notifyChanges(ReloadMode mode) {
		...
		if(mode == ReloadMode.GAME) {
			mc.displayGuiScreen(new MultiChoiceScreen(T -> mc.displayGuiScreen(owner), Texts.translatable("gui.carbonconfig.restart.title"), Texts.translatable("gui.carbonconfig.restart.message").setChatStyle(Texts.applyStyle(EnumChatFormatting.GRAY)), Texts.translatable("gui.carbonconfig.ok")));
		}
		else if(mode == ReloadMode.WORLD && mc.theWorld != null) {
			mc.displayGuiScreen(new MultiChoiceScreen(T -> mc.displayGuiScreen(owner), Texts.translatable("gui.carbonconfig.reload.title"), Texts.translatable("gui.carbonconfig.reload.message").setChatStyle(Texts.applyStyle(EnumChatFormatting.GRAY)), Texts.translatable("gui.carbonconfig.ok")));
		}
	}
}
