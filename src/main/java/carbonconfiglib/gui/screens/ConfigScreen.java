...
public class ConfigScreen extends BaseCarbonScreen implements IElementContext, IFolderController
{
	...
	public ConfigScreen(IModConfig configs, BackgroundHolder holder, Screen parent) {
		...
		display = Texts.literal(FabricLoader.getInstance().getModContainer(configs.getModId()).map(ModContainer::getMetadata).map(ModMetadata::getName).orElse(configs.getModId())).append(" -> ").append(configs.getConfigName());
	...
	private void notifyChanges(ReloadMode mode) {
		...
		if(mode == ReloadMode.GAME) {
			minecraft.setScreen(new MultiChoiceScreen(T -> minecraft.setScreen(owner), Texts.translatable("gui.carbonconfig.restart.title"), Texts.translatable("gui.carbonconfig.restart.message").withStyle(ChatFormatting.GRAY), Texts.translatable("gui.carbonconfig.ok")));
		}
		else if(mode == ReloadMode.WORLD && minecraft.level != null) {
			minecraft.setScreen(new MultiChoiceScreen(T -> minecraft.setScreen(owner), Texts.translatable("gui.carbonconfig.reload.title"), Texts.translatable("gui.carbonconfig.reload.message").withStyle(ChatFormatting.GRAY), Texts.translatable("gui.carbonconfig.ok")));
		}
	}
}
