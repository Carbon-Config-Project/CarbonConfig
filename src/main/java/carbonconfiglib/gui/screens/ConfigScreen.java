...
public class ConfigScreen extends BaseCarbonScreen implements IElementContext, IFolderController
{
	...
	Screen parent;
	TextState searchState = new TextState().setCallback(this::applySearch).setMaxLength(255).setSuggestion(I18n.format("gui.carbonconfig.search"));
	...
	public ConfigScreen(IModConfig configs, BackgroundHolder holder, Screen parent) {
		...
		display = Texts.literal(ModList.get().getModContainerById(configs.getModId()).map(ModContainer::getModInfo).map(IModInfo::getDisplayName).orElse(configs.getModId())).appendText(" -> ").appendText(configs.getConfigName());
	...
	public void renderForeground(int mouseX, int mouseY, float partialTicks) {
		...
		if(!rowOne.isScrollbarVisible() && !rowTwo.isEmpty()) {
			Screen.fill(widthOne, (int)(height * 0.15F), widthOne+4, (int)(height * 0.8F), 0xFF000000);
		}
		int widthTwo = calculateWidth(1) + widthOne + 4;
		if(rowTwo.isVisible() && !rowTwo.isScrollbarVisible() && !rowThree.isEmpty()) {
			Screen.fill(widthTwo, (int)(height * 0.15F), widthTwo+4, (int)(height * 0.8F), 0xFF000000);
		}
		if(rowThree.isVisible()) {
			int widthThree = (int)(width * totalWidth()) - 4;
			Screen.fill(widthThree, (int)(height * 0.15F), widthThree+4, (int)(height * 0.8F), 0xFF000000);
		}
		Screen.fill(0, (int)(height * 0.8F), width, height, 0xFF1C1C1C);
		boolean found = false;
		for(int i = 2;i>=0;i--) {
			if(all[i].isVisible()) {
		...
		Screen.fill(0, (int)(height * 0.85F), width, (int)(height * 0.85F+2), 0xFF000000);
	}
	...
	private void drawTooltip(BaseElement element) {
		...
		if(text != null) {
			...
			GuiUtils.drawText(font, text, 0F, 0F, Align.START, -1);
	...
	private void notifyChanges(ReloadMode mode) {
		...
		if(mode == ReloadMode.GAME) {
			minecraft.displayGuiScreen(new MultiChoiceScreen(T -> minecraft.displayGuiScreen(owner), Texts.translatable("gui.carbonconfig.restart.title"), Texts.translatable("gui.carbonconfig.restart.message").applyTextStyle(TextFormatting.GRAY), Texts.translatable("gui.carbonconfig.ok")));
		}
		else if(mode == ReloadMode.WORLD && minecraft.world != null) {
			minecraft.displayGuiScreen(new MultiChoiceScreen(T -> minecraft.displayGuiScreen(owner), Texts.translatable("gui.carbonconfig.reload.title"), Texts.translatable("gui.carbonconfig.reload.message").applyTextStyle(TextFormatting.GRAY), Texts.translatable("gui.carbonconfig.ok")));
		}
	}
}
