...
public class ConfigScreen extends BaseCarbonScreen implements IElementContext, IFolderController
{
	...
	public ConfigScreen(IModConfig configs, BackgroundHolder holder, Screen parent) {
		...
		display = Texts.literal(ModList.get().getModContainerById(configs.getModId()).map(ModContainer::getModInfo).map(IModInfo::getDisplayName).orElse(configs.getModId())).append(" -> ").append(configs.getConfigName());
	...
	public void renderForeground(MatrixStack matrix, int mouseX, int mouseY, float partialTicks) {
		...
		if(!rowOne.isScrollbarVisible() && !rowTwo.isEmpty()) {
			Screen.fill(matrix, widthOne, (int)(height * 0.15F), widthOne+4, (int)(height * 0.8F), 0xFF000000);
		}
		int widthTwo = calculateWidth(1) + widthOne + 4;
		if(rowTwo.isVisible() && !rowTwo.isScrollbarVisible() && !rowThree.isEmpty()) {
			Screen.fill(matrix, widthTwo, (int)(height * 0.15F), widthTwo+4, (int)(height * 0.8F), 0xFF000000);
		}
		if(rowThree.isVisible()) {
			int widthThree = (int)(width * totalWidth()) - 4;
			Screen.fill(matrix, widthThree, (int)(height * 0.15F), widthThree+4, (int)(height * 0.8F), 0xFF000000);
		}
		Screen.fill(matrix, 0, (int)(height * 0.8F), width, height, 0xFF1C1C1C);
		boolean found = false;
		for(int i = 2;i>=0;i--) {
			if(all[i].isVisible()) {
		...
		Screen.fill(matrix, 0, (int)(height * 0.85F), width, (int)(height * 0.85F+2), 0xFF000000);
	}
	
	private void drawTooltip(BaseElement element, MatrixStack matrix) {
	...
	private void notifyChanges(ReloadMode mode) {
		...
		if(mode == ReloadMode.GAME) {
			minecraft.setScreen(new MultiChoiceScreen(T -> minecraft.setScreen(owner), Texts.translatable("gui.carbonconfig.restart.title"), Texts.translatable("gui.carbonconfig.restart.message").withStyle(TextFormatting.GRAY), Texts.translatable("gui.carbonconfig.ok")));
		}
		else if(mode == ReloadMode.WORLD && minecraft.level != null) {
			minecraft.setScreen(new MultiChoiceScreen(T -> minecraft.setScreen(owner), Texts.translatable("gui.carbonconfig.reload.title"), Texts.translatable("gui.carbonconfig.reload.message").withStyle(TextFormatting.GRAY), Texts.translatable("gui.carbonconfig.ok")));
		}
	}
}
