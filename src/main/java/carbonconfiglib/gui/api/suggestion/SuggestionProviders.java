...
public class SuggestionProviders
{
	public static class ModProvider implements ISuggestionProvider {
		...
		public void provideSuggestions(Consumer<Suggestion> output, Predicate<Suggestion> filter) {
			for(ModContainer info : FabricLoader.getInstance().getAllMods()) {
				ModMetadata data = info.getMetadata();
				Suggestion suggestion = Suggestion.namedValue(data.getName(), data.getId());
	...
	public static class PlayerProvider implements ISuggestionProvider {
		...
		private void collectProfiles(Consumer<GameProfile> profile) {
			...
			try {
				JsonArray array = new JsonParser().parse(Files.newBufferedReader(FabricLoader.getInstance().getGameDir().resolve("usercache.json"))).getAsJsonArray();
