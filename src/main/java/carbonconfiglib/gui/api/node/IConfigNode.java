...
public interface IConfigNode
{
	...
	public static IChatComponent createLabel(String name, String translationKey) {
		return translationKey != null && Texts.hasKey(translationKey) ? Texts.translatable(translationKey) : createLabel(name);
	}
	
	public static IChatComponent createLabel(String name) {
		...
		for(String s : name.split("\\-|\\_|(?<!^)(?=[A-Z][a-z])|(?<!(^|[A-Z]))(?=[A-Z])")) {
			...
			comp.appendText(s.toLowerCase().replaceFirst(first, first.toUpperCase()).trim()).appendText(" ");
		}
		return comp;
	}
}
