...
public class CarbonArray implements IArrayNode, IValueActions
{
	...
	public CarbonArray(String nodeName, ConfigPath path, IReloadMode mode, ListData data, IChatComponent name, IChatComponent tooltip, String currentValue, String defaultValue, Function<String, ParseResult<Boolean>> isValid, Supplier<List<Suggestion>> suggestions, BiConsumer<String, IValueActions> saveAction) {
	...
	protected IValueActions addEntry(String value, String defaultValue, int index) {
		switch(inner.getDataType()) {
			case COMPOUND: return new CarbonCompound(Integer.toString(index), path.append("array"), mode, inner.asCompound(), name.createCopy().appendText(index+": "), tooltip, value, defaultValue, this::isValid, () -> data.getSuggestions(T -> true), this::save).setAutosave(true);
			case LIST: return new CarbonArray(Integer.toString(index), path.append("array"), mode, inner.asList(), name.createCopy().appendText(index+": "), tooltip, value, defaultValue, this::isValid, () -> data.getSuggestions(T -> true), this::save).setAutosave(true);
			case SIMPLE: return new CarbonValue(null, mode, name.createCopy().appendText(index+": "), tooltip, null, inner, data.isForced(), () -> data.getSuggestions(T -> true), value, defaultValue, this::isValid, this::save).setAutosave(true);
			default: return null;
		}
	}
