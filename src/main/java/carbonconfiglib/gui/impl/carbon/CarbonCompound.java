...
public class CarbonCompound implements ICompoundNode, IValueActions
{
	...
	public CarbonCompound(String nodeName, ConfigPath path, IReloadMode mode, CompoundData data, IChatComponent name, IChatComponent tooltip, String value, String defaultValue, Function<String, ParseResult<Boolean>> isValid, Supplier<List<Suggestion>> suggestions, BiConsumer<String, IValueActions> saveAction) {
	...
	private IChatComponent createTooltip(String key) {
		...
		if(entryKey != null && Texts.hasKey(entryKey)) {
			comp.appendSibling(Texts.translatable(entryKey).setChatStyle(Texts.applyStyle(EnumChatFormatting.YELLOW))).appendText("\n");
		}
		else {
