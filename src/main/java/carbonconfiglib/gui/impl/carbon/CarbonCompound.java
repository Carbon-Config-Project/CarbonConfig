...
public class CarbonCompound implements ICompoundNode, IValueActions
{
	...
	public CarbonCompound(String nodeName, ConfigPath path, IReloadMode mode, CompoundData data, ITextComponent name, ITextComponent tooltip, String value, String defaultValue, Function<String, ParseResult<Boolean>> isValid, Supplier<List<Suggestion>> suggestions, BiConsumer<String, IValueActions> saveAction) {
	...
	private ITextComponent createTooltip(String key) {
		...
		if(entryKey != null && I18n.hasKey(entryKey)) {
			comp.appendSibling(Texts.translatable(entryKey).setStyle(Texts.applyStyle(TextFormatting.YELLOW))).appendText("\n");
		}
		else {
