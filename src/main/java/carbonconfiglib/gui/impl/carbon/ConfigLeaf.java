...
public class ConfigLeaf implements IConfigNode
{
	...
	public ITextComponent getTooltip() {
		...
		if(key != null && I18n.hasKey(key)) {
			comp.appendSibling(Texts.translatable(key).setStyle(Texts.applyStyle(TextFormatting.GRAY)).appendText("\n"));
		}
		else {
			String[] array = entry.getComment();
			if(array != null && array.length > 0) {
				for(int i = 0;i<array.length;comp.appendText(array[i++]).setStyle(Texts.applyStyle(TextFormatting.GRAY)).appendText("\n"));
			}
		}
		...
		if(!Strings.isBlank(limit)) {
			if(limit.contains("\nExample:")) {
				...
				for(String entry : limit.split("\n")) {
					...
					result.appendSibling(Texts.literal(entry).setStyle(Texts.applyStyle(current))).appendText("\n");
				}
				comp.appendSibling(result);
			}
			else comp.appendSibling(Texts.literal(limit).setStyle(Texts.applyStyle(TextFormatting.BLUE)));
		}
		return comp;
	}
	@Override
