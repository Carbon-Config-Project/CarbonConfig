		...
		if(!Strings.isBlank(limit)) {
			if(limit.contains("\nExample:")) {
				...
				for(String entry : limit.split("\n")) {
					...
					result.appendSibling(Texts.literal(entry).applyTextStyle(current)).appendText("\n");
				}
				comp.appendSibling(result);
			}
			else comp.appendSibling(Texts.literal(limit).applyTextStyle(TextFormatting.BLUE));
		}
		return comp;
	}
	@Override
