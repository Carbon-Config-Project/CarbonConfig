...
public class MinecraftConfig implements IModConfig
{
	...
	public byte[] createBackup() {
		...
		try { CompressedStreamTools.writeCompressed(original.write(), stream); }
	...
	public void loadBackup(byte[] data) {
		try {
			current.read(CompressedStreamTools.readCompressed(new ByteArrayInputStream(data)));
			save(false);
		}
		catch(Exception e) { e.printStackTrace(); }
	}
