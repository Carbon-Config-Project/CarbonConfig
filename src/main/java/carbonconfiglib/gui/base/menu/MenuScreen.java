...
public class MenuScreen extends BaseCarbonScreen {
	...
	public void drawArea(float x, float y, float widht, float height) {
		...
		WorldRenderer buffer = tes.getWorldRenderer();
		...
		fillGradient(minX - 3, minY - 4, maxX + 3, minY - 3, -267386864, buffer);
		fillGradient(minX - 3, maxY + 3, maxX + 3, maxY + 4, -267386864, buffer);
		fillGradient(minX - 3, minY - 3, maxX + 3, maxY + 3, -267386864, buffer);
		fillGradient(minX - 4, minY - 3, minX - 3, maxY + 3, -267386864, buffer);
		fillGradient(maxX + 3, minY - 3, maxX + 4, maxY + 3, -267386864, buffer);
		
		fillGradient(minX - 3, minY - 3 + 1, minX - 3 + 1, maxY + 3 - 1, 1347420415, buffer);
		fillGradient(maxX + 2, minY - 3 + 1, maxX + 3, maxY + 3 - 1, 1347420415, buffer);
		fillGradient(minX - 3, minY - 3, maxX + 3, minY - 3 + 1, 1347420415, buffer);
		fillGradient(minX - 3, maxY + 2, maxX + 3, maxY + 3, 1344798847, buffer);
	...
	protected void fillGradient(float left, float top, float right, float bottom, int color, WorldRenderer buffer) {
		float a = (color >> 24 & 255) / 255F;
		float r = (color >> 16 & 255) / 255F;
		float g = (color >> 8 & 255) / 255F;
		float b = (color & 255) / 255F;
		buffer.pos(right, top, -10F).color(r, g, b, a).endVertex();
		buffer.pos(left, top, -10F).color(r, g, b, a).endVertex();
		buffer.pos(left, bottom, -10F).color(r, g, b, a).endVertex();
		buffer.pos(right, bottom, -10F).color(r, g, b, a).endVertex();
	}
	...
	public static class MenuEntry extends ListEntry<MenuEntry> {
		...
		private static int height(IMenuItem item, FontRenderer font) {
			return height(GuiUtils.splitLines(font, item.name().getFormattedText(), 180).size());
		}
		...
		private static int width(IMenuItem item, FontRenderer font)  {
			int width = 0;
			for(String entry : GuiUtils.splitLines(font, item.name().getFormattedText(), 180)) {
		...
		public void render(int x, int top, int left, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
			lastY = top;
			List<String> sequence = GuiUtils.splitLines(font, item.name().getFormattedText(), 180);
