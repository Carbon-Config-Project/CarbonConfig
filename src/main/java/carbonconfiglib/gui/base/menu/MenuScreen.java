...
public class MenuScreen extends BaseCarbonScreen {
	...
	public void drawArea(MatrixStack matrix, float x, float y, float widht, float height) {
	...
	protected void fillGradient(MatrixStack matrix, float left, float top, float right, float bottom, int color, BufferBuilder buffer) {
	...
	public static class MenuEntry extends ListEntry<MenuEntry> {
		...
		private static int width(IMenuItem item, FontRenderer font)  {
			int width = 0;
			for(IReorderingProcessor entry : font.split(item.name(), 180)) {
		...
		public void render(MatrixStack poseStack, int x, int top, int left, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
			lastY = top;
			List<IReorderingProcessor> sequence = font.split(item.name(), 180);
			...
			for(IReorderingProcessor entry : sequence) {
