...
public class GuiUtils
{
	...
	public static void drawText(GuiGraphics graphics, Font font, Component text, float x, float y, Align align, int color) {
		...
		drawString(font, graphics, text.getVisualOrderText(), x + offset, y, color, false);
	}
	...
	public static void drawSplitText(GuiGraphics graphics, Font font, Component text, float x, float y, Align align, int color, int maxLength, float lineSplit) {
		for(FormattedCharSequence line : font.split(text, maxLength)) {
			...
			drawString(font, graphics, line, x + offset, y, color, false);
			y += lineSplit;
		}
	}
	...
	public static void drawScrollingText(GuiGraphics graphics, Font font, Component text, float x, float y, float width, float height, Align align, int color, long startTime) {
		...
		if(textWidth > width) {
			...
			drawString(font, graphics, text.getVisualOrderText(), x - align.align(width) + align.align(textWidth) + (float)Mth.lerp(offset, 0D, diff), y + (height * 0.5F) - (font.lineHeight * 0.5F), color, false);
			popScissors();
			return;
		}
		float offset = align.align(textWidth);
		drawString(font, graphics, text.getVisualOrderText(), x - align.align(width) + offset, y + (height * 0.5F) - (font.lineHeight * 0.5F), color, false);
	}
	...
	public static void drawScrollingShadowText(GuiGraphics graphics, Font font, Component text, float x, float y, float width, float height, Align align, int color, long startTime) {
		...
		if(textWidth > width) {
			...
			double offset = Math.sin((Math.PI / 2D) * Math.cos(((Math.PI * 2D) * timer) / minDiff)) / 2D + 0.01F + align.alignCenter();
			pushScissors((int)x, (int)y, (int)width, (int)height);
			drawString(font, graphics, text.getVisualOrderText(), x - align.align(width) + align.align(textWidth) + (float)Mth.lerp(offset, 0D, diff), y + (height * 0.5F) - (font.lineHeight * 0.5F), color, true);
			popScissors();
			return;
		}
		float offset = align.align(textWidth);
		drawString(font, graphics, text.getVisualOrderText(), x - align.align(width) + offset, y + (height * 0.5F) - (font.lineHeight * 0.5F), color, true);
	}
	
	private static int drawString(Font font, GuiGraphics graphics, FormattedCharSequence text, float x, float y, int color, boolean shadow) {
		int width = font.drawInBatch(text, x, y, color, shadow, graphics.pose().last().pose(), graphics.bufferSource(), DisplayMode.NORMAL, 0, 15728880);
		graphics.flush();
		return width;
	}
