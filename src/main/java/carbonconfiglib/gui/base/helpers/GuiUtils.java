...
public class GuiUtils
{
	...
	public static void drawQuadArea(float left, float top, float right, float bottom, WorldRenderer builder, int color) {
		if(left < right) {
			float i = left;
			left = right;
			right = i;
	...
	public static void drawLine(float startX, float startY, float endX, float endY, float width, WorldRenderer builder, int color) {
		float f3 = (float)(color >> 24 & 255) / 255.0F;
		float f = (float)(color >> 16 & 255) / 255.0F;
		float f1 = (float)(color >> 8 & 255) / 255.0F;
		float f2 = (float)(color & 255) / 255.0F;
