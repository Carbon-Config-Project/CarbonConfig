...
public class GuiUtils
{
	...
	public static void drawText(FontRenderer font, String text, float x, float y, Align align, int color) {
		...
		font.drawString(text, x + offset, y, color);
	}
	...
	public static void drawSplitText(FontRenderer font, String text, float x, float y, Align align, int color, int maxLength, float lineSplit) {
		for(String line : font.listFormattedStringToWidth(text, maxLength)) {
			float offset = align.align(font.getStringWidth(line));
			font.drawString(line, x + offset, y, color);
			y += lineSplit;
		}
	}
	...
	public static void drawScrollingText(FontRenderer font, String text, float x, float y, float width, float height, Align align, int color, long startTime) {
		...
		if(textWidth > width) {
			...
			font.drawString(text, x - align.align(width) + align.align(textWidth) + (float)MathHelper.lerp(offset, 0D, diff), y + (height * 0.5F) - (font.FONT_HEIGHT * 0.5F), color);
			popScissors();
			return;
		}
		float offset = align.align(textWidth);
		font.drawString(text, x - align.align(width) + offset, y + (height * 0.5F) - (font.FONT_HEIGHT * 0.5F), color);
	}
	...
	public static void drawScrollingShadowText(FontRenderer font, String text, float x, float y, float width, float height, Align align, int color, long startTime) {
		...
		if(textWidth > width) {
			...
			double offset = Math.sin((Math.PI / 2D) * Math.cos(((Math.PI * 2D) * timer) / minDiff)) / 2D + 0.01F + align.alignCenter();
			pushScissors((int)x, (int)y, (int)width, (int)height);
			font.drawStringWithShadow(text, x - align.align(width) + align.align(textWidth) + (float)MathHelper.lerp(offset, 0D, diff), y + (height * 0.5F) - (font.FONT_HEIGHT * 0.5F), color);
			popScissors();
			return;
		}
		float offset = align.align(textWidth);
	...
	public static void drawQuadArea(float left, float top, float right, float bottom, IVertexConsumer builder, int color) {
		if(left < right) {
			float i = left;
			left = right;
			right = i;
	...
	public static void drawLine(float startX, float startY, float endX, float endY, float width, IVertexConsumer builder, int color) {
		float f3 = (float)(color >> 24 & 255) / 255.0F;
		float f = (float)(color >> 16 & 255) / 255.0F;
		float f1 = (float)(color >> 8 & 255) / 255.0F;
		float f2 = (float)(color & 255) / 255.0F;
