...
public class LayeredScreen extends Screen
{
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        Matrix4f matrix4f = new Matrix4f().setOrtho(0.0F, (float)((double)window.getWidth() / window.getGuiScale()), (float)((double)window.getHeight() / window.getGuiScale()), 0.0F, 1000.0F, farPlane);
        RenderSystem.setProjectionMatrix(matrix4f, VertexSorting.ORTHOGRAPHIC_Z);
        Matrix4fStack poseStack = RenderSystem.getModelViewStack();
        poseStack.identity();
        poseStack.translation(0.0F, 0.0F, 1000 - farPlane);
		...
		forEach(T -> {
			stack.pushPose();
			T.render(graphics, Integer.MAX_VALUE, Integer.MAX_VALUE, partialTicks);
			stack.popPose();
			stack.translate(0F, 0F, 2000F);
		}, false);
		layers.top().render(graphics, mouseX, mouseY, partialTicks);
		stack.popPose();
	}
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
		return !layers.isEmpty() && layers.top().mouseScrolled(mouseX, mouseY, scrollX, scrollY);
	}
