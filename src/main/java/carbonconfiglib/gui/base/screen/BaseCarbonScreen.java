		...
		if(mouseX != Integer.MAX_VALUE && mouseY != Integer.MAX_VALUE) {
			for(IInteractable listener : children()) {
				if(listener instanceof ITooltipProvider) {
					((ITooltipProvider)listener).provideTooltips(mouseX, mouseY, T -> tooltips.addAll(GuiUtils.splitLines(fontRenderer, T, Math.max(mouseX, width - mouseX) - 20)));
				}
			}
			collectTooltips(mouseX, mouseY, partialTicks, T -> tooltips.addAll(GuiUtils.splitLines(fontRenderer, T, Math.max(mouseX, width - mouseX) - 20)));
		}
