...
public class BaseCarbonScreen extends GuiScreen implements IInteractableContainer, ITickableScreen
{
		...
		if(mouseX != Integer.MAX_VALUE && mouseY != Integer.MAX_VALUE) {
			for(IInteractable listener : children()) {
				if(listener instanceof ITooltipProvider) {
					((ITooltipProvider)listener).provideTooltips(mouseX, mouseY, T -> tooltips.addAll(fontRendererObj.listFormattedStringToWidth(T.getFormattedText(), Math.max(mouseX, width - mouseX) - 20)));
				}
			}
			collectTooltips(mouseX, mouseY, partialTicks, T -> tooltips.addAll(fontRendererObj.listFormattedStringToWidth(T.getFormattedText(), Math.max(mouseX, width - mouseX) - 20)));
		}
	...
	public void drawText(IChatComponent text, float x, float y, Align align, int color) {
		GuiUtils.drawText(fontRendererObj, text, x + centerX, y + centerY, align, color);
	}
	
	public void drawUnalignedText(IChatComponent text, float x, float y, Align align, int color) {
		GuiUtils.drawText(fontRendererObj, text, x, y, align, color);
	}
	
	public void drawSplitText(IChatComponent text, float x, float y, Align align, int maxWidth, int color) {
		GuiUtils.drawSplitText(fontRendererObj, text, x + centerX, y + centerY, align, color, maxWidth);
	}
	
	public CarbonButton button(int x, int y, int width, int height, IChatComponent text, Consumer<CarbonBaseButton> listener) {
		return addWidget(new CarbonButton(x, y, width, height, text, listener));
	}
	
	public CarbonButton button(int x, int y, int width, int height, Align horizontal, Align vertical, IChatComponent text, Consumer<CarbonBaseButton> listener) {
