...
public class BaseCarbonScreen extends GuiScreen implements IInteractableContainer, ITickableScreen
{
		...
		if(mouseX != Integer.MAX_VALUE && mouseY != Integer.MAX_VALUE) {
			for(IInteractable listener : children()) {
				if(listener instanceof ITooltipProvider) {
					((ITooltipProvider)listener).provideTooltips(mouseX, mouseY, T -> tooltips.addAll(fontRenderer.listFormattedStringToWidth(T.getFormattedText(), Math.max(mouseX, width - mouseX) - 20)));
				}
			}
			collectTooltips(mouseX, mouseY, partialTicks, T -> tooltips.addAll(fontRenderer.listFormattedStringToWidth(T.getFormattedText(), Math.max(mouseX, width - mouseX) - 20)));
		}
	...
	public void drawText(ITextComponent text, float x, float y, Align align, int color) {
		GuiUtils.drawText(fontRenderer, text, x + centerX, y + centerY, align, color);
	}
	
	public void drawUnalignedText(ITextComponent text, float x, float y, Align align, int color) {
		GuiUtils.drawText(fontRenderer, text, x, y, align, color);
	}
	
	public void drawSplitText(ITextComponent text, float x, float y, Align align, int maxWidth, int color) {
		GuiUtils.drawSplitText(fontRenderer, text, x + centerX, y + centerY, align, color, maxWidth);
	}
	...
	public CarbonEditBox text(int x, int y, int width, int height, TextState state) {
		return addWidget(new CarbonEditBox(fontRenderer, x, y, width, height, state));
	}
	
	public CarbonEditBox text(int x, int y, int width, int height, Align horizontal, Align vertical, TextState state) {
		return addWidget(new CarbonEditBox(fontRenderer, getAlignedX(horizontal) + x, getAlignedY(vertical) + y, width, height, state));
	}
