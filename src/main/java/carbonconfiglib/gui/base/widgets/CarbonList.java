...
public class CarbonList<T extends ListEntry<T>> extends CarbonDynamicList<T> implements ITooltipProvider {
	...
	public static abstract class ListEntry<T extends ListEntry<T>> extends CarbonDynamicList.DynamicEntry<T> implements ITooltipProvider {
		...
		protected <E extends IGuiEventListener> E addChild(E child) {
			children.add(child);
			return child;
		}
		...
		@Override
		public abstract void render(MatrixStack poseStack, int x, int top, int left, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks);
	}
