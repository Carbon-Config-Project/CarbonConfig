...
public class CarbonList<T extends ListEntry<T>> extends CarbonDynamicList<T> implements ITooltipProvider {
	...
	public CarbonList(Screen owner, int width, int height, int y, ListState<T> state) {
		super(Minecraft.getInstance(), width, height, y, state.getItemHeight());
		this.state = state;
		state.setOwner(this);
	...
	public CarbonList(Screen owner, ListState<T> state) {
		this(owner, owner.width, owner.height, state.getTopPadding(), state);
	}
