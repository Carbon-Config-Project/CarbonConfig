...
public class CarbonList<T extends ListEntry<T>> extends CarbonDynamicList<T> implements ITooltipProvider {
	...
	public CarbonList(int width, int height, int startY, int endY, ListState<T> state) {
	...
	public CarbonList(GuiScreen owner, ListState<T> state) {
		this(owner.width, owner.height, state.getTopPadding(), owner.height-state.getBottomPadding(), state);
	}
	...
	public void setScrollAmount(double value, boolean force) {
		float actualValue = (float)Helpers.clamp(value, 0, getMaxScroll());
	...
	public static abstract class ListEntry<T extends ListEntry<T>> extends CarbonDynamicList.DynamicEntry<T> implements ITooltipProvider, IInteractableContainer {
