...
public class CarbonButton extends CarbonBaseButton {
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		if(selected) {
			...
			GuiUtils.blitWithBorder(BUTTON_TEXTURES, xPosition, yPosition, 0, 46 + k * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, zLevel, true);
			GlStateManager.color(1F, 1F, 1F, 1F);
		}
		else GuiUtils.blitWithBorder(BUTTON_TEXTURES, xPosition, yPosition, 0, 46 + k * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, zLevel, false);
		if(highlighted) {
		...
		GuiUtils.drawScrollingShadowText(Minecraft.getMinecraft().fontRendererObj, displayString, xPosition+2, yPosition+2, width-4, height-4, Align.CENTER, this.enabled ? 16777215 : 10526880, 0);
