...
public class CarbonButton extends CarbonBaseButton {
	...
	public void renderWidget(GuiGraphics graphics, int pMouseX, int pMouseY, float pPartialTick) {
		...
		if(selected) {
			...
			GuiUtils.blitWithBorder(graphics, WIDGETS_LOCATION, getX(), getY(), 0, 46 + k * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, true);
			RenderSystem.setShaderColor(1F, 1F, 1F, 1F);
		}
		else GuiUtils.blitWithBorder(graphics, WIDGETS_LOCATION, getX(), getY(), 0, 46 + k * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, false);
		if(highlighted) {
