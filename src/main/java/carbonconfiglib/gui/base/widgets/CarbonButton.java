...
public class CarbonButton extends CarbonBaseButton {
	...
	public void renderButton(int pMouseX, int pMouseY, float pPartialTick) {
		...
		if(selected) {
			...
			GuiUtils.blitWithBorder(WIDGETS_LOCATION, x, y, 0, 46 + k * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, this.getBlitOffset(), true);
			RenderSystem.color4f(1F, 1F, 1F, 1F);
		}
		else GuiUtils.blitWithBorder(WIDGETS_LOCATION, x, y, 0, 46 + k * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, this.getBlitOffset(), false);
		if(highlighted) {
