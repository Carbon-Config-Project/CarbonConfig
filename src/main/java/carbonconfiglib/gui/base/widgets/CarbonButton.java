...
public class CarbonButton extends CarbonBaseButton {
	...
	public CarbonButton(int pX, int pY, int pWidth, int pHeight, ITextComponent pMessage, Consumer<CarbonBaseButton> listener) {
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		this.hovered = mousePressed(Minecraft.getMinecraft(), mouseX, mouseY);
		int k = this.getHoverState(this.isHovered());
		if(selected) {
			GlStateManager.color(0.5F, 0.5F, 0.5F, 1.0F);
			GuiUtils.blitWithBorder(BUTTON_TEXTURES, x, y, 0, 46 + k * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, zLevel, true);
			GlStateManager.color(1F, 1F, 1F, 1F);
		}
		else GuiUtils.blitWithBorder(BUTTON_TEXTURES, x, y, 0, 46 + k * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, zLevel, false);
		if(highlighted) {
			Gui.drawRect(x+2, y+2, x+getButtonWidth()-2, y+getHeight()-2, 0x33FFFFFF);
		}
		
		if (icon.isPresent()) {
			renderIcon(mouseX, mouseY, partialTicks);
		}
		GuiUtils.drawScrollingShadowText(Minecraft.getMinecraft().fontRenderer, displayString, x+2, y+2, width-4, height-4, Align.CENTER, this.enabled ? 16777215 : 10526880, 0);
