...
public class CarbonButton extends CarbonBaseButton {
	...
	public void renderIcon(PoseStack pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
		...
		GuiUtils.drawTextureRegion(pPoseStack, getX() + padding, getY() + padding, width-padding*2, height-padding*2, icon.get(), 16, 16);
	...
	public void renderButton(PoseStack pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
		...
		if(selected) {
			...
			GuiUtils.blitWithBorder(pPoseStack, WIDGETS_LOCATION, getX(), getY(), 0, 46 + k * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, this.getBlitOffset(), true);
			RenderSystem.setShaderColor(1F, 1F, 1F, 1F);
		}
		else GuiUtils.blitWithBorder(pPoseStack, WIDGETS_LOCATION, getX(), getY(), 0, 46 + k * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, this.getBlitOffset(), false);
		if(highlighted) {
			Screen.fill(pPoseStack, getX()+2, getY()+2, getX()+getWidth()-2, getY()+getHeight()-2, 0x33FFFFFF);
		}
		
		if (icon.isPresent()) {
			renderIcon(pPoseStack, pMouseX, pMouseY, pPartialTick);
		}
		GuiUtils.drawScrollingShadowText(pPoseStack, Minecraft.getInstance().font, getMessage(), getX()+2, getY()+2, width-4, height-4, Align.CENTER, this.active ? 16777215 : 10526880, hash);
	}
}
