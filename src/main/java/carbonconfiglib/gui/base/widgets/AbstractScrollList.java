...
import carbonconfiglib.gui.base.widgets.AbstractScrollList.Entry;
...
public class AbstractScrollList<E extends Entry<E>> implements IInteractableContainer, IWidget
{
	...
	protected final E getEntryAtPosition(double mouseX, double mouseY) {
		...
		int k = j - i;
		int l = j + i;
		int i1 = (int)Math.floor(mouseY - (double)this.y0) - this.headerHeight + (int)this.getScrollAmount() - 4;
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		if (this.renderBackground) {
			...
			Minecraft.getMinecraft().getTextureManager().bindTexture(GuiScreen.OPTIONS_BACKGROUND);
			int color = 32;
	...
	public void setScrollAmount(double value) {
		scroll = MathHelper.clamp_double(value, 0.0D, (double)this.getMaxScroll());
	}
