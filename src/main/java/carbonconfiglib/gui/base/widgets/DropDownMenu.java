...
public class DropDownMenu<T> extends CarbonButton {
	...
	public void renderButton(MatrixStack poseStack, int mouseX, int mouseY, float partialTick) {
		if(state.isSimpleButton()) {
			...
			if(icon != null) {
				...
				GuiUtils.blitWithBorder(poseStack, WIDGETS_LOCATION, this.x, this.y, 0, 46 + k * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, this.getBlitOffset(), false);
		...
		GuiUtils.blitWithBorder(poseStack, WIDGETS_LOCATION, this.x, this.y, 0, 46 + k * 20, this.width-14, this.height, 200, 20, 2, 3, 2, 2, this.getBlitOffset(), false);
		GuiUtils.blitWithBorder(poseStack, WIDGETS_LOCATION, this.x+width-15, this.y, 0, 46 + k * 20, 15, this.height, 200, 20, 2, 3, 2, 2, this.getBlitOffset(), false);
		GuiUtils.drawScrollingShadowText(poseStack, mc.font, getMessage(), x+2, y, width-18, height, Align.CENTER, getFGColor(), hash);
		GuiUtils.drawScrollingShadowText(poseStack, mc.font, Texts.literal(open ? (up ? "▲" : "▼") : "◀"), x+width-13, y, 11, height, Align.CENTER, getFGColor(), hash);
	}
	...
	public static class DropDownScreen<T> extends BaseCarbonScreen {
		...
		public void renderSelection(MatrixStack matrix, int left, int top, int width, int height, int frameColor, int backgroundColor) {
	...
	public static class DropDownState<T> {
		...
		protected ITextComponent generateText() {
			return (valueOnly ? Texts.empty() : prefix.copy().append(": ")).append(selected.size() == 0 ? empty : (selected.size() == 1 ? displayFunction.apply(selected.get(0)) : Texts.translatable("gui.carbonconfig.dropdown.selected_elements", selected.size())));
		}
