...
public class DropDownMenu<T> extends CarbonButton {
	...
	protected boolean clicked(double pMouseX, double pMouseY) {
		return this.enabled && this.visible && pMouseX >= (double)this.x && pMouseY >= (double)this.y && pMouseX < (double)(this.x + this.width) && pMouseY < (double)(this.y + this.height);
	}
	...
	public void render(int mouseX, int mouseY, float partialTick) {
		if(state.isSimpleButton()) {
			...
			if(icon != null) {
				...
				GuiUtils.blitWithBorder(BUTTON_TEXTURES, this.x, this.y, 0, 46 + k * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, zLevel, false);
		...
		int k = this.getHoverState(this.hovered || open);
		int j = this.enabled ? 16777215 : 10526880;
		GuiUtils.blitWithBorder(BUTTON_TEXTURES, this.x, this.y, 0, 46 + k * 20, this.width-14, this.height, 200, 20, 2, 3, 2, 2, zLevel, false);
		GuiUtils.blitWithBorder(BUTTON_TEXTURES, this.x+width-15, this.y, 0, 46 + k * 20, 15, this.height, 200, 20, 2, 3, 2, 2, zLevel, false);
		GuiUtils.drawScrollingShadowText(mc.fontRenderer, displayString, x+2, y, width-18, height, Align.CENTER, j, 0);
		GuiUtils.drawScrollingShadowText(mc.fontRenderer, Texts.literal(open ? (up ? "▲" : "▼") : "◀"), x+width-13, y, 11, height, Align.CENTER, j, 0);
	}
