	...
	public void renderWidget(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
		if(state.isSimpleButton()) {
			...
			if(icon != null) {
		    	...
		    	GuiUtils.drawTextureRegion(poseStack, getX()+2, getY()+2, width-4, height-4, state.getIcon(), state.iconWidth, state.iconHeight);
		    	return;
			}
		...
		int k = this.getTextureY(this.isHovered || open);
		ScreenUtils.blitWithBorder(poseStack, WIDGETS_LOCATION, this.getX(), this.getY(), 0, 46 + k * 20, this.width-14, this.height, 200, 20, 2, 3, 2, 2, 0);
		ScreenUtils.blitWithBorder(poseStack, WIDGETS_LOCATION, this.getX()+width-15, this.getY(), 0, 46 + k * 20, 15, this.height, 200, 20, 2, 3, 2, 2, 0);
		GuiUtils.drawScrollingShadowText(poseStack, mc.font, getMessage(), getX()+2, getY(), width-18, height, Align.CENTER, getFGColor(), hash);
		GuiUtils.drawScrollingShadowText(poseStack, mc.font, Component.literal(open ? (up ? "▲" : "▼") : "◀"), getX()+width-13, getY(), 11, height, Align.CENTER, getFGColor(), hash);
	}
