...
public class DropDownMenu<T> extends CarbonButton {
	...
	public void renderButton(int mouseX, int mouseY, float partialTick) {
		if(state.isSimpleButton()) {
			...
			if(icon != null) {
				...
				GuiUtils.blitWithBorder(WIDGETS_LOCATION, this.x, this.y, 0, 46 + k * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, this.getBlitOffset(), false);
				RenderSystem.enableDepthTest();
				RenderSystem.color4f(1.0F, 1.0F, 1.0F, this.alpha);
				RenderSystem.enableBlend();
				RenderSystem.defaultBlendFunc();
				RenderSystem.blendFunc(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA);
		    	GuiUtils.drawTextureRegion(x+2, y+2, width-4, height-4, state.getIcon(), state.iconWidth, state.iconHeight);
		    	return;
			}
		...
		GuiUtils.blitWithBorder(WIDGETS_LOCATION, this.x, this.y, 0, 46 + k * 20, this.width-14, this.height, 200, 20, 2, 3, 2, 2, this.getBlitOffset(), false);
		GuiUtils.blitWithBorder(WIDGETS_LOCATION, this.x+width-15, this.y, 0, 46 + k * 20, 15, this.height, 200, 20, 2, 3, 2, 2, this.getBlitOffset(), false);
		GuiUtils.drawScrollingShadowText(mc.fontRenderer, getMessage(), x+2, y, width-18, height, Align.CENTER, getFGColor(), hash);
		GuiUtils.drawScrollingShadowText(mc.fontRenderer, Texts.literal(open ? (up ? "▲" : "▼") : "◀"), x+width-13, y, 11, height, Align.CENTER, getFGColor(), hash);
	}
	...
	public static class DropDownScreen<T> extends BaseCarbonScreen {
		...
		private void generateElements() {
			...
			Function<T, ITextComponent> text = ownerState.displayFunction;
	...
	public static class DropDownEntry<T> extends ListEntry<DropDownEntry<T>> {
		...
		public DropDownEntry(T data, ITextComponent text, ListEntry<?> renderer) {
			this.data = data;
	...
	public static class DropDownState<T> {
		...
		int iconWidth = 16;
		int iconHeight = 16;
		ITextComponent prefix = Texts.translatable("gui.carbonconfig.dropdown.selected");
		ITextComponent empty = Texts.translatable("gui.carbonconfig.dropdown.none");
		Consumer<List<T>> listener;
		int xOffset = 0;
		OptionalInt customWidth = OptionalInt.empty();
		
		@SafeVarargs
		public DropDownState(Function<T, ITextComponent> displayFunction, T... values) {
			this(displayFunction, Arrays.asList(values));
		}
		
		public DropDownState(Function<T, ITextComponent> displayFunction, List<T> values) {
		...
		@SafeVarargs
		public DropDownState(T selected, Function<T, ITextComponent> displayFunction, T... values) {
			this(selected, displayFunction, Arrays.asList(values));
		}
		
		public DropDownState(T selected, Function<T, ITextComponent> displayFunction, List<T> values) {
			this.displayFunction = displayFunction;
			this.values.addAll(values);
		...
		@SafeVarargs
		public DropDownState(List<T> selection, Function<T, ITextComponent> displayFunction, T... values) {
			this(selection, displayFunction, Arrays.asList(values));
		}
		
		public DropDownState(List<T> selection, Function<T, ITextComponent> displayFunction, List<T> values) {
			this.displayFunction = displayFunction;
			this.values.addAll(values);
		...
		public DropDownState<T> withPrefix(ITextComponent prefix) {
			this.prefix = Objects.requireNonNull(prefix);
			updateText();
			return this;
		}
		
		public DropDownState<T> withEmpty(ITextComponent empty) {
		...
		public void updateText() {
			...
			owner.setMessage(generateText().getFormattedText());
		}
		
		protected ITextComponent generateText() {
			return (valueOnly ? Texts.empty() : prefix.deepCopy().appendText(": ")).appendSibling(selected.size() == 0 ? empty : (selected.size() == 1 ? displayFunction.apply(selected.get(0)) : Texts.translatable("gui.carbonconfig.dropdown.selected_elements", selected.size())));
		}
