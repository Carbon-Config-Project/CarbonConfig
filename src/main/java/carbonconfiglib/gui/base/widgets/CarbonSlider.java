...
public class CarbonSlider extends CarbonBaseButton {
	...
	public void render(int pMouseX, int pMouseY, float pPartialTick) {
		...
		GuiUtils.blitWithBorder(BUTTON_TEXTURES, x, y, 0, 46 + i * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, zLevel, false);
		
		this.renderBg(mc, pMouseX, pMouseY);
		int j = this.enabled ? 16777215 : 10526880;
		GuiUtils.drawScrollingText(font, displayString, x, y+1, width, height, Align.CENTER, j | 255 << 24, 0);
	}
	...
	protected void renderBg(Minecraft mc, int mouseX, int mouseY) {
		...
		GuiUtils.blitWithBorder(BUTTON_TEXTURES, this.x + (int)(range * (float)(this.width - 8)), this.y, 0, 46 + i, 8, this.height, 200, 20, 2, 3, 2, 2, zLevel, true);
	}
