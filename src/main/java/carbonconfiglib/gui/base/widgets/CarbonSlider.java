...
public class CarbonSlider extends CarbonBaseButton {
	...
	public void renderButton(PoseStack pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
	      ...
	      GuiUtils.blitWithBorder(pPoseStack, WIDGETS_LOCATION, getX(), getY(), 0, 46 + i * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, this.getBlitOffset(), false);

	      this.renderBg(pPoseStack, minecraft, pMouseX, pMouseY);
	      int j = getFGColor();
	      GuiUtils.drawScrollingText(pPoseStack, font, getMessage(), getX(), getY()+1, width, height, Align.CENTER, j | Mth.ceil(this.alpha * 255.0F) << 24, 0);
	}
	...
	protected void renderBg(PoseStack stack, Minecraft mc, int mouseX, int mouseY) {
		...
		GuiUtils.blitWithBorder(stack, WIDGETS_LOCATION, this.getX() + (int)(range * (float)(this.width - 8)), this.getY(), 0, 46 + i, 8, this.height, 200, 20, 2, 3, 2, 2, this.getBlitOffset(), true);
	}
