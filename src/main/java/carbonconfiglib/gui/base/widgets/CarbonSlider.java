...
public class CarbonSlider extends CarbonBaseButton {
	...
	public void updateMessage() {
		displayString = state.getCurrentDisplayText().getFormattedText();
	}
	...
	protected void setFromMouse(double mouseX) {
		double progress = Math.max(0, Math.min(1, (mouseX - (xPosition + 4D)) / (width - 8D)));
	...
	public boolean mouseScroll(double mouseX, double mouseY, double scroll) {
		if(state.stepSize != 0 && enabled && visible) {
			state.set(get() + (long)(state.stepSize * scroll * (GuiScreen.isShiftKeyDown() ? 10D : 1D) * (GuiScreen.isCtrlKeyDown() ? 100D : 1D)));
			return true;
		}
		return false;
	}
	...
	public boolean charTyped(char character, int keyCode) {
		if(state.stepSize != 0 && enabled && visible && (keyCode == Keyboard.KEY_LEFT || keyCode == Keyboard.KEY_RIGHT)) {
			state.set(get() + (state.stepSize * ((keyCode == Keyboard.KEY_LEFT ? -1L : 0L) + (keyCode == Keyboard.KEY_RIGHT ? 1L : 0L)) * (GuiScreen.isShiftKeyDown() ? 10L : 1L) * (GuiScreen.isCtrlKeyDown() ? 100L : 1L)));
			return true;
		}
		return false;		
	}
	...
	public void render(int pMouseX, int pMouseY, float pPartialTick) {
		...
		GuiUtils.blitWithBorder(buttonTextures, xPosition, yPosition, 0, 46 + i * 20, this.width, this.height, 200, 20, 2, 3, 2, 2, zLevel, false);
		
		this.renderBg(mc, pMouseX, pMouseY);
		int j = this.enabled ? 16777215 : 10526880;
		GuiUtils.drawScrollingText(font, displayString, xPosition, yPosition+1, width, height, Align.CENTER, j | 255 << 24, 0);
	}
	
	protected void renderBg(Minecraft mc, int mouseX, int mouseY) {
		mc.getTextureManager().bindTexture(buttonTextures);
		if(enabled) GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		else GL11.glColor4f(0.5F, 0.5F, 0.5F, 1.0F);
		int i = !enabled ? 20 : (this.isHovered() ? 2 : 1) * 20;
		double range = getProgress();
		GuiUtils.blitWithBorder(buttonTextures, this.xPosition + (int)(range * (float)(this.width - 8)), this.yPosition, 0, 46 + i, 8, this.height, 200, 20, 2, 3, 2, 2, zLevel, true);
	}
	...
	public static class SliderState {
		...
		long value;
		long stepSize = 1L;
		LongFunction<IChatComponent> displayText = T -> Texts.literal(NUMBERS.format(T));
		IChatComponent prefix = Texts.empty();
		IChatComponent suffix = Texts.empty();
		...
		public SliderState(long value, long minValue, long maxValue, LongFunction<IChatComponent> displayText) {
			this.minValue = minValue;
			this.maxValue = maxValue;
			this.value = value;
		...
		public SliderState(long value, long minValue, long maxValue, IChatComponent prefix, IChatComponent suffix) {
			this.minValue = minValue;
			this.maxValue = maxValue;
			this.value = value;
		...
		public SliderState(long value, long minValue, long maxValue, LongFunction<IChatComponent> displayText, IChatComponent prefix, IChatComponent suffix) {
			this.minValue = minValue;
			this.maxValue = maxValue;
			this.value = value;
			this.displayText = Objects.requireNonNull(displayText);
		...
		public SliderState setDisplayFunction(LongFunction<IChatComponent> displayText) {
			this.displayText = Objects.requireNonNull(displayText);
			return this;
		}
		...
		public SliderState setPrefix(IChatComponent prefix) {
			this.prefix = Objects.requireNonNull(prefix);
			if(owner != null) owner.updateMessage();
			return this;
		}
		
		public SliderState setSuffix(IChatComponent suffix) {
			this.suffix = Objects.requireNonNull(suffix);
			if(owner != null) owner.updateMessage();
			return this;
		}
		
		public IChatComponent getCurrentDisplayText() {
