...
public class CarbonCheckBox extends CarbonBaseButton implements ITooltipProvider {
	...
	public void provideTooltips(int mouseX, int mouseY, Consumer<ITextComponent> tooltips) {
		if (state.tooltip != null && canShowTooltip(mouseX, mouseY)) {
			ITextComponent result = state.tooltip.apply(this);
	...
	public void renderButton(int pMouseX, int pMouseY, float pPartialTick) {
		...
		GuiUtils.drawTextureRegion(x, y, isMouseOver(pMouseX, pMouseY) ? 20F : 0F, !notVanilla && selected() ? 20F : 0F, width, height, 20F, 20F, 64F, 64F);
		if(notVanilla) {
			GuiUtils.drawTextureRegion(x+2, y+2, width-4, height-4, selected() ? state.getIcon().active() : state.getIcon().inactive(), 16, 16);
		}
		if (state.label != null) {
			drawString(Minecraft.getInstance().fontRenderer, state.label.getFormattedText(), this.x + 24, this.y + (this.height - 8) / 2, 14737632 | MathHelper.ceil(this.alpha * 255.0F) << 24);
		}
	}

	public static class CheckBoxState {
		protected Function<CarbonCheckBox, ITextComponent> tooltip;
		...
		public CheckBoxState(ITextComponent label) {
			this.label = label;
		}
		
		public CheckBoxState(boolean value, ITextComponent label) {
			this.value = value;
			this.label = label;
		}
		...
		public CheckBoxState withLabel(ITextComponent label) {
			this.label = label;
			return this;
		}
		...
		public CheckBoxState setTooltip(ITextComponent tooltip) {
			this.tooltip = T -> tooltip;
			return this;
		}

		public CheckBoxState withTooltip(Function<CarbonCheckBox, ITextComponent> tooltip) {
