...
public class CarbonCheckBox extends Checkbox implements ITooltipProvider {
	...
	public void renderButton(PoseStack pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
		...
		GuiUtils.drawTextureRegion(pPoseStack, getX(), getY(), isMouseOver(pMouseX, pMouseY) ? 20F : 0F, !notVanilla && selected() ? 20F : 0F, width, height, 20F, 20F, 64F, 64F);
		if(notVanilla) {
			GuiUtils.drawTextureRegion(pPoseStack, getX()+2, getY()+2, width-4, height-4, selected() ? state.getIcon().active() : state.getIcon().inactive(), 16, 16);
		}
		if (state.label != null) {
			drawString(pPoseStack, Minecraft.getInstance().font, state.label, this.getX() + 24, this.getY() + (this.height - 8) / 2, 14737632 | Mth.ceil(this.alpha * 255.0F) << 24);
		}
	}
