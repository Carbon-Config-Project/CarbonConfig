...
public class CarbonCheckBox extends CarbonBaseButton implements ITooltipProvider {
	...
	public void render(int mouseX, int mouseY, float partialTicks){
		...
		GuiUtils.drawTextureRegion(x, y, isMouseOver(mouseX, mouseY) ? 20F : 0F, !notVanilla && selected() ? 20F : 0F, width, height, 20F, 20F, 64F, 64F);
		if(notVanilla) {
			GuiUtils.drawTextureRegion(x+2, y+2, width-4, height-4, selected() ? state.getIcon().active() : state.getIcon().inactive(), 16, 16);
		}
		if (state.label != null) {
			drawString(mc.fontRenderer, state.label.getFormattedText(), this.x + 24, this.y + (this.height - 8) / 2, 14737632 | 255 << 24);
		}
	}
