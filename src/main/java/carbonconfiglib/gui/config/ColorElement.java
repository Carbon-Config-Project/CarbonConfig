...
public class ColorElement extends ConfigElement
{
	...
	public void render(GuiGraphics poseStack, int x, int top, int left, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks)
	{
		...
		if(isArray()) {
			poseStack.fill(left+186, top-1, left+203, top+19, 0xFFA0A0A0);
			poseStack.fill(left+187, top, left+202, top+18, Integer.decode(value.get()) | 0xFF000000);
		}
		else {
			int xOff = isCompound() ? 106 : 186;
			poseStack.fill(left+xOff, top-1, left+xOff+17, top+19, 0xFFA0A0A0);
			poseStack.fill(left+xOff+1, top, left+xOff+16, top+18, Integer.decode(value.get()) | 0xFF000000);
		}
