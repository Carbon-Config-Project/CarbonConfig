                    ...
                    while (it.hasNext()) {
                        ...
                        if (a.getType() == AbstractInsnNode.FRAME) {
                            ...
                            if (f.type == Opcodes.F_APPEND && f.local.size() == 1 && f.local.get(0).equals("net/minecraft/client/network/NetHandlerPlayClient")) {
                                InsnList list = new InsnList();
                                list.add(new VarInsnNode(Opcodes.ALOAD, 0));
                                list.add(new FieldInsnNode(Opcodes.GETFIELD, "net/minecraft/client/Minecraft", method.name.equals("loadWorld") ? "integratedServer" : "field_71437_Z", "Lnet/minecraft/server/integrated/IntegratedServer;"));
