...
public class BackupManager {
	...
	private static Path appendWorld(Path path) {
		...
		;
		return path.resolve(server.getFile(".").toPath().getParent().getFileName()).resolve(Long.toHexString(server.getWorld(DimensionType.OVERWORLD).getSeed()));
	}
	
	private static Path appendServer(Path path) {
		return path.resolve(sanitizeName(Minecraft.getInstance().getIntegratedServer().getName()));
	}
			...
			if(entry.getValue().test(buf)) {
				switch(mode) {
					...
					case LOAD:
						...
						break;
					case LIST:
						BaseCarbonScreen.setExternalScreen(new BackupSelectionScreen(Minecraft.getInstance().currentScreen, BackgroundTexture.DEFAULT.asHolder(), entry.getKey(), BackupManager.listBackups(entry.getKey())));
						break;
				}
				if(toCheck.isEmpty()) {
