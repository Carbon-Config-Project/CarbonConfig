			...
			if(entry.getValue().test(buf)) {
				switch(mode) {
					...
					case LOAD:
						...
						break;
					case LIST:
						BaseCarbonScreen.pushExternalScreen(new BackupSelectionScreen(Minecraft.getInstance().currentScreen, BackgroundTexture.DEFAULT.asHolder(), entry.getKey(), BackupManager.listBackups(entry.getKey())));
						break;
				}
				if(toCheck.isEmpty()) {
