...
public class BackupManager {
	...
	public static class BulkRequest implements IRequestReceiver {
		public static final SystemToastId BACKUP_HINT = new SystemToastId(1000);
		Mode mode;
			...
			if(entry.getValue().test(buf)) {
				...
				if(toCheck.isEmpty()) {
					if(CarbonConfig.BACKUP_TOASTS.get()) {
						if(mode == Mode.CREATE) Minecraft.getInstance().getToasts().addToast(new SystemToast(BACKUP_HINT, Component.translatable("gui.carbonconfig.toast.create"), Component.translatable("gui.carbonconfig.toast.create.desc")));
						else if(mode == Mode.LOAD) Minecraft.getInstance().getToasts().addToast(new SystemToast(BACKUP_HINT, Component.translatable("gui.carbonconfig.toast.load"), Component.translatable("gui.carbonconfig.toast.create.load")));
					}
