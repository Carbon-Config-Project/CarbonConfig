...
public class NamedRegistry<T>
{
	...
	public static final NamedRegistry<Potion> MOB_EFFECTS = NamedRegistry.ofForge(GameData.getPotionRegistry(), T -> T.getName());
	...
	public static <T> NamedRegistry<T> ofForge(FMLControlledNamespacedRegistry<T> registry, Function<T, String> nameFunction) {
		return new NamedRegistry<T>(nameFunction, 
				registry::getKeys, registry::getNameForObject, registry::containsKey, registry::getId, T -> registry.getNameForObject(registry.getObjectById(T)), 
				() -> registry, registry::getObject, T -> registry.containsKey(registry.getNameForObject(T)), registry::getId, registry::getObjectById);
	}
