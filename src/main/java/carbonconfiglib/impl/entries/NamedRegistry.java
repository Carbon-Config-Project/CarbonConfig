...
public class NamedRegistry<T>
{
	...
	public static final NamedRegistry<Item> ITEMS = NamedRegistry.ofForge(ForgeRegistries.ITEMS, T -> T.getItemStackDisplayName(new ItemStack(T)));
	...
	public static <T extends IForgeRegistryEntry<T>> NamedRegistry<T> ofForge(IForgeRegistry<T> registry, Function<T, String> nameFunction) {
		FMLControlledNamespacedRegistry<T> impl = (FMLControlledNamespacedRegistry<T>)registry;
		return new NamedRegistry<T>(nameFunction, 
				registry::getKeys, registry::getKey, registry::containsKey, impl::getId, T -> impl.getKey(impl.getObjectById(T)), 
				() -> registry, registry::getValue, registry::containsValue, impl::getId, impl::getObjectById);
	}
