...
public class NamedRegistry<T>
{
	...
	public static final NamedRegistry<Fluid> FLUID = NamedRegistry.ofForge(ForgeRegistries.FLUIDS, T -> T.getFluidType().getDescription().getString());
	public static final NamedRegistry<MobEffect> MOB_EFFECTS = NamedRegistry.ofForge(ForgeRegistries.MOB_EFFECTS, T -> T.getDisplayName().getString());
	...
	public static <T> NamedRegistry<T> ofForge(IForgeRegistry<T> registry, Function<T, String> nameFunction) {
		ForgeRegistry<T> impl = (ForgeRegistry<T>)registry;
		return new NamedRegistry<T>(nameFunction, 
				registry::getKeys, registry::getKey, registry::containsKey, impl::getID, T -> Optional.ofNullable(impl.getKey(T)).map(ResourceKey::location).orElse(null), 
