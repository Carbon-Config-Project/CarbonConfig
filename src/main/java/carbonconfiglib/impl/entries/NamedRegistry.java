...
{
	public static final NamedRegistry<Block> BLOCKS = NamedRegistry.ofForge(ForgeRegistries.BLOCKS, T -> T.getNameTextComponent().getString());
	public static final NamedRegistry<Item> ITEMS = NamedRegistry.ofForge(ForgeRegistries.ITEMS, T -> T.getName().getString());
	...
	public static final NamedRegistry<Enchantment> ENCHANTMENTS = NamedRegistry.ofForge(ForgeRegistries.ENCHANTMENTS, T -> T.getDisplayName(0).shallowCopy().getString());
	...
	public static <T> NamedRegistry<T> ofRegistry(Registry<T> registry, Function<T, String> nameFunction) {
		return new NamedRegistry<T>(nameFunction, 
				registry::keySet, registry::getKey, registry::containsKey, T -> registry.getId(registry.getOrDefault(T)), T -> registry.getKey(registry.getByValue(T)), 
				() -> registry, registry::getOrDefault, T -> registry.containsKey(registry.getKey(T)), registry::getId, registry::getByValue);
	}
