...
public class ColorValue extends BasicConfigEntry<ColorWrapper>
{
	...
	public ColorValue(String key, int defaultValue, String... comment) {
		this(key, defaultValue, false, comment);
	}
	
	public ColorValue(String key, int defaultValue, boolean includeAlpha, String... comment) {
		super(key, new ColorWrapper(defaultValue), comment);
		this.includeAlpha = includeAlpha;
		addSettings(new ColorType(includeAlpha));
	}
	
	public ColorValue withAlpha(boolean value) {
		addSettings(new ColorType(value));
		return this;
	}
	
	@Override
	protected ColorValue copy() {
		return new ColorValue(getKey(), get(), includeAlpha, getComment());
	}
	...
	public static void addMCColorPalette(Consumer<Suggestion> result, Predicate<Suggestion> filter) {
		for(TextFormatting formatting : TextFormatting.values()) {
			...
			Suggestion value = Suggestion.namedTypeValue(Helpers.firstLetterUppercase(formatting.name()), ColorWrapper.serializeRGB(formatting.getColor()), ColorWrapper.class);
			if(filter.test(value)) result.accept(value);
		}
	}
	...
	protected String serializedValue(MultilinePolicy policy, ColorWrapper value) {
		return includeAlpha ? ColorWrapper.serialize(value.getColor()) : ColorWrapper.serializeRGB(value.getColor());
	}
	...
	public static ParseResult<ColorValue> parse(String key, String value, String... comment) {
		...
		return ParseResult.success(new ColorValue(key, result.getValue(), (result.getValue() & 0xFF000000) != 0, comment));
	}
	...
	public static class ColorWrapper extends Number {
		...
		public static String serializeNumberRGB(long color) {
			return (Long.toHexString(0xFF000000L | (color & 0xFFFFFFL)).substring(2));
		}
		
		public static String serializeNumber(long color) {
			return (Long.toHexString(0xFF00000000L | (color & 0xFFFFFFFFL)).substring(2));
		}
		...
		public static String serializeHTMLRGB(long color) {
			return "#"+(Long.toHexString(0xFF000000L | (color & 0xFFFFFFL)).substring(2));
		}
		
		public static String serializeHTML(long color) {
			return "#"+(Long.toHexString(0xFF00000000L | (color & 0xFFFFFFFFL)).substring(2));
		}
	}
}
